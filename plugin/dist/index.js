...
function splitSelectedModelRef(selectedModel) {
...
function describeRouteRequestFailure(result, timeoutMs) {
...
const recentRouteModes = new Map();
const recentConversationRouteModes = new Map();
...
const pendingOutcomeQueueBySessionId = new Map();
...
const recentFailedOverrides = new Map();
const RECENT_COMMAND_GUARD_MS = 15_000;
let recentLastDecisionGlobal = null;
function rememberRecentUserMessage(sessionKey, text) {
...
export function isShortFollowUpForContextualRouting(text) {
    ...
    if (trimmed.length <= 24)
        return true;
    if (words.length <= 5 && trimmed.length <= 48)
        return true;
    return false;
}
export function shouldUseContextualLlmClassifier(routeMode, conversationContext, routingText) {
...
async function loadPersistedRouteModePreference(routerUrl, key, scope = "conversation") {
    const trimmedKey = key.trim();
    if (!trimmedKey)
        return null;
    try {
...
function rememberConversationKeyForSession(sessionKey, conversationKey) {
...
function hasRecentFeedbackPrompt(decisionId) {
    const trimmed = String(decisionId ?? "").trim();
    if (!trimmed)
        return false;
...
function summarizeExcludedModels(excludedModels, limit = 3) {
 ...
 */
export function stripOpenClawMetadataEnvelope(text) {
    ...
    let stripped = text
        ...
        .replace(/```json\s*\{[\s\S]*?(?:"message_id"|"sender_id"|"schema":\s*"openclaw)[\s\S]*?```/gi, "");
    // Collapse runs of blank lines
    stripped = stripped.replace(/\n{3,}/g, "\n\n").trim();
    return stripped;
}
function shouldSuppressFeedbackCardForText(messagePreview) {
    ...
    const looksLikeApprovalRelay = lowered.includes("/approve")
...
function markRecentStartup(sessionKey, reason = "startup") {
    if (!sessionKey)
        return;
    recentStartupBySession.set(sessionKey, { at: Date.now(), reason });
}
function takeRecentStartupReason(sessionKey) {
    if (!sessionKey)
        return null;
...
function shouldDefaultRouteOff(sessionKey) {
    const kind = classifySessionKind(sessionKey);
    return kind === "direct" || kind === "slash" || kind === "main";
}
function inferCronLabel(prompt) {
    const lowered = prompt.toLowerCase();
    if (lowered.includes("self_heal_alerts.py") || lowered.includes("self-heal incident"))
        return "self-heal";
    if (lowered.includes("morning-brief.sh") || lowered.includes("morning brief"))
        return "morning-brief";
    if (lowered.includes("backup status"))
...
function clearRecentRoutingState(sessionKey) {
    if (!sessionKey)
        return;
...
function resetInMemoryRoutingState() {
...
function buildFailedOverrideKeys(sessionKey, conversationKey, selectedModel, selectedProvider, scope = "both") {
    const keys = new Set();
    const model = selectedModel.trim();
    const provider = selectedProvider.trim();
    if (sessionKey) {
        if (scope === "model" || scope === "both")
...
function failedOverrideScopeForReason(reason) {
    return reason === "timeout" ? "model" : "both";
}
function rememberFailedOverride(sessionKey, conversationKey, selectedModel, selectedProvider, reason) {
    const now = Date.now();
...
function getFailedOverrideBlock(sessionKey, conversationKey, selectedModel, selectedProvider) {
    const now = Date.now();
...
function chooseUnblockedFallback(fallbacks, sessionKey, conversationKey) {
...
function inferFailureFromRuntime(event) {
...
function takeRecentConversationContext(sessionKey) {
    if (!sessionKey)
        return null;
...
function resolveConversationKeyForSession(sessionKey) {
    if (!sessionKey)
        return null;
...
function resolveSessionKeyForConversation(conversationKey) {
...
function shouldBypassCompiledRetryRouting(ctx, source, sessionKey) {
    if (source !== "compiled-prompt")
        return false;
    if (ctx?.trigger !== "user")
        return false;
    if (!sessionKey)
...
function shouldBlockRoutingForBurst(sessionRef) {
    if (!sessionRef)
        return false;
    const now = Date.now();
...
function buildConversationKeyFromContext(ctx) {
    ...
    if (!channel && !fromTo && !thread)
        return null;
    return [channel ?? "unknown", account, fromTo, thread].join(":");
}
function takeRecentUserMessage(sessionKey) {
    if (!sessionKey)
        return null;
...
function getRecentRouteModeEntry(sessionKey) {
    if (!sessionKey)
        return null;
...
function takeRecentRouteMode(sessionKey) {
    return getRecentRouteModeEntry(sessionKey)?.mode ?? null;
}
function getRecentConversationRouteModeEntry(conversationKey) {
    if (!conversationKey)
        return null;
...
function takeRecentConversationRouteMode(conversationKey) {
...
function rememberLastDecision(sessionKey, decision) {
    if (sessionKey) {
...
function enqueuePendingOutcome(sessionId, pending) {
    const queue = pendingOutcomeQueueBySessionId.get(sessionId) ?? [];
    queue.push(pending);
    pendingOutcomeQueueBySessionId.set(sessionId, queue);
}
function peekPendingOutcome(sessionId) {
    const queue = pendingOutcomeQueueBySessionId.get(sessionId);
    if (!queue?.length)
        return undefined;
    return queue[0];
}
function shiftPendingOutcome(sessionId) {
    const queue = pendingOutcomeQueueBySessionId.get(sessionId);
    if (!queue?.length)
        return undefined;
...
function takeLastDecisionForConversation(conversationKey) {
...
function extractEffectiveTaskType(reason) {
...
function buildRouteLastReply(last) {
    if (!last) {
        return {
...
function buildRouteExplainReply(last) {
    if (!last) {
        return {
            text: "No recent routing decision found yet for this conversation.",
        };
...
async function buildRouteCompareReply(routerUrl, last, modes, timeoutMs) {
    ...
    const results = await Promise.all(uniqueModes.map(async (mode) => {
    ...
    for (const row of results) {
        ...
        lines.push(`${row.mode}: ${row.decision.selected_model} ` +
...
function resolveCostProfileForRouteMode(mode, defaultProfile) {
    switch (mode) {
        case "auto":
...
function buildRouteInteractiveReply(mode, scopeLabel = "this conversation", freeFilter, includeInteractive = true) {
...
function resolveLastDecisionForContext(ctx, conversationKey) {
...
function buildRouteHelpText(currentMode) {
    return [
        `⚙️ Nexus Router help`,
...
function collectTextFragments(value, output, seen = new WeakSet()) {
    ...
    if (Array.isArray(value)) {
        for (const item of value)
            collectTextFragments(item, output, seen);
        return;
    }
...
function buildConversationContextFromMessages(messages) {
    const fragments = [];
...
async function resolveRouteModeFromSession(api, sessionKey) {
    return (await resolveRouteModeDetailsFromContext(api, { sessionKey })).mode;
}
async function resolveRouteModeDetailsFromContext(api, ctx) {
    const candidates = [];
    const seen = new Set();
    const routerUrl = (api?.config ?? {}).routerUrl ?? DEFAULT_URL;
    const addConversationCandidate = (key, source) => {
        if (!key || seen.has(`${source}:${key}`))
            return;
        seen.add(`${source}:${key}`);
...
function shouldAutoEscalate(confidence) {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        if (!enabled) {
            if (debugMode)
                console.log("[nexus-router] disabled via config");
            return;
        }
        ...
        api.on("before_model_resolve", async (event, ctx) => {
            ...
            if (routeMode === "off") {
                ...
                const lastDecision = {
                    ...
                    classifierSource: (shadowDecision.classifier_source ?? (shouldUseLlmClassifier ? "llm" : "heuristic")),
            ...
            if (!decision) {
                ...
                if (debugMode)
                    console.warn("[nexus-router] router unavailable, using default model");
                return;
            }
            const blockedOverride = getFailedOverrideBlock(ctx.sessionKey ?? sessionRef, conversationKey, decision.selected_model, decision.selected_provider);
            if (blockedOverride) {
                ...
                if (fallbackModel) {
                    ...
                    await debugLog(`[hook-result] source=${source} source_tag=${sourceTag} route=${finalMode} blocked model=${decision.selected_model} fallback=${fallbackModel} reason=${blockedOverride.reason}`);
                    decision = {
                        ...decision,
                    ...
                    await debugLog(`[hook-result] source=${source} source_tag=${sourceTag} route=${finalMode} blocked model=${decision.selected_model} fallback=none reason=${blockedOverride.reason}`);
                    return;
                }
            }
            if (sessionRef) {
            ...
            const lastDecision = {
                ...
                classifierSource: (decision.classifier_source ?? (shouldUseLlmClassifier ? "llm" : "heuristic")),
            ...
            if (shouldEnforceConfidenceGate && decision.confidence < minConfidence) {
                ...
                await debugLog(`[hook-result] source=${source} source_tag=${sourceTag} skipped confidence=${decision.confidence.toFixed(2)} threshold=${minConfidence}`);
                if (debugMode) {
                    console.log(`[nexus-router] confidence ${decision.confidence.toFixed(2)} < ${minConfidence}, skipping`);
                }
                return;
            }
            ...
            await debugLog(`[hook-result] source=${source} source_tag=${sourceTag} route=${finalMode} override model=${decision.selected_model} task=${decision.task_type} confidence=${decision.confidence.toFixed(2)} score=${decision.score.toFixed(3)}`);
            if (debugMode) {
                console.log(`[nexus-router] ${decision.task_type} → ${decision.selected_model}` +
        ...
        api.on("llm_output", async (event, ctx) => {
            ...
            const bySessionKey = (pending?.sessionKey ?? ctx.sessionKey)
                ? recentLastDecisionBySession.get((pending?.sessionKey ?? ctx.sessionKey))
                : undefined;
        ...
        api.on("agent_end", async (event, ctx) => {
            ...
            try {
                ...
                if (failure.shouldCooldownOverride && !event.success) {
                    rememberFailedOverride(pending.sessionKey ?? ctx.sessionKey, failedConversationKey, outcomeModel, outcomeProvider, failure.errorType ?? "unknown");
                }
                ...
                if (pending.shadowMode) {
                    ...
                    if (decision?.decisionId && sender?.senderId) {
                        await sendTelegramFeedbackCard(api, sender.senderId, {
                        ...
                        }, decision.sourceTag, { shadowMode: true, actualModel: outcomeModel, messagePreview: decision.promptText });
                    }
                    else {
                        await debugLog(`[feedback-card] shadow skipped decision=${pending.decisionId} reason=${decision ? 'missing_sender' : 'missing_decision'}`);
                    }
                }
            }
            catch {
